...
int main()
{
	...
	next: main:																		//goto 문의 도착 지점
		...
		printf("17. Overcalc의 역사와 발전\n");
		printf("18. Pro버전 제품키 받기\n");
		printf("19. Pro버전으로 업그레이드\n");
		printf("20. 나가기\n");
		MENU_BAR
		printf("번호를 입력하세요 : ");
		scanf_s("%d", &qvsf);
		...
		case 10: 
			...
			case 13: 
				...
				while (TRUE)
				{
					sutja_menu(&SUTJA); scanf_s("%c", &num, 1);
		...
		case 18:
			...
			for (z = 0; z < 29; z++)							//z가 0이고, z가 29 미만일 동안 출력 - 총 29개의 문자를 출력
			{
		...
		case 19: 
			...
			break;
		case 20: exit(FALSE);
		case 999: Easter_Egg(&honor); break;
	...
	pro:
		...
		printf("15. 원주율 출력하기\n");
		printf("16. Overcalc의 역사와 발전\n");				
		printf("17. Premium 제품키 받기(준비중)\n");
		printf("18. Premium으로 업그레이드(준비중)\n");
		printf("Q. 나가기\n");
		PRO_BAR
		printf("번호를 입력하세요 : ");
		scanf_s("%hhd", &num);//char형 정수는 %hhd
		...
		case 10: 
			...
			while (KABA.jumsu<10 && KABA.jumsu>-3)
			{
				kababo_menu(&KABA); scanf_s("%hhd", &num);
		...
		case 17:CLS
			if (honor < 1000)
			{
				printf("\n명예점수가 1000점 이상인 경우에만 가능합니다.\n"); PUS break;
			}
				
				printf("이 키를 복사하시면 PREMIUM버전을 이용하실 수 있습니다.\n");
				for (zl = 0; zl < 17; zl++)
				{
					if ((zl + 1) % 3 == 0 && zl != 0 && (pl == 0))			//zl이 0이 아니고 zl에 1을 더한 값이 4로 나누어 떨어지고 pl이 0이라면
					{
						printf("-");																	//(숫자 혹은 영문자2개 출력후) '-' 출력
						enter[zl] = '-';															//제품키의 zl부분에 '-' 문자를 삽입
						++pl;																		//zp는 1이 되어 이 if 문에 접근이 불가
					}

					else if (zl != 0 && pl == 1 && (zl + 1) % 14 == 0)		//zl이 0이 아니고 zl에 1을 더한 값이 14로 나누어 떨어지고 zp가 0이라면
					{
						printf("-");																	//(숫자 혹은 영문자 6개 추가 출력후) 두번째 '-' 출력
						enter[zl] = '-';															//제품키의 zl부분에 '-' 문자를 삽입
						++pl;																		//zp는 2가되어 이 else if 문에 접근이 불가
					}

					else if (zl != 0 && pl == 2 && (zl + 1) % 20 == 0)		//zl이 0이 아니고 zl에 1을 더한 값이 11로 나누어 떨어지고 zp가 0이라면
					{
						printf("-");																	//(숫자 혹은 영문자 6개 추가 출력후) 두번째 '-' 출력
					...
					else if ((zl + 1) % 3 == 0)											//그게 또 아니라면 zl에 1을 더한 값이 4로 나누어 떨어 진다면
					{
						qwer = rand() % 8 + 49;											//49~57 = char 1~9(정수가 아닌 문자)
						printf("%c", qwer);														//를 출력하고
						enter[zl] = qwer;														//zl부분에 숫자 삽입
					}
				}
				/*끝났다면 enter배열이 완성된 것임*/
				printf("\n\n이 키는 비상용 키로, 인증은 되지 않지만 PREMIUM버전의 일부를 사용할 수 있습니다.\n");

								//인증이 안되는 제품키
				printf("%s\n", non_key);
				ll break;

		case 18:while (TRUE)												//그냥 1
		{
			printf("제품키를 입력하세요...");
			scanf_s("%s", enter2, sizeof(enter2));
			if (!strcmp(enter2, enter))
			{
				activation(); goto premium;
			}

			else if (!strcmp(key_real, "Q3H4JRO3MXC89R"))
				goto nonpro;
	...
	while (TRUE)
	{
		...
		CLS C_3F
		PRO_BAR		PRE_TITLE		HONOR		PRO_BAR
		printf("1. 나의 나이는 0000년에 몇 살일까?\n");
		printf("2. 타이머\n");											//나이 계산기
		printf("3. 라면 분석기\n");									//라면 시간 계산기
		printf("4. 진짜 계산기(1회용)\n");
			...
			while (TRUE)
		{
			...
			{
				activation(); goto enterprise;
			}
			else if (!strcmp(act_in, "exit"))
				break;
	...
	enterprise:
		...
		CLS C_3F
		PRO_BAR		ENT_TITLE		HONOR		PRO_BAR
		printf("1. 나의 나이는 0000년에 몇 살일까?\n");
		printf("2. 타이머\n");											//나이 계산기
		printf("3. 라면 분석기\n");									//라면 시간 계산기
		printf("4. 진짜 계산기(1회용)\n");
		...
		printf("15. 원주율 출력하기\n");
		printf("16. Overcalc의 역사와 발전\n");
		printf("17. Ultimate 제품키 받기(준비중)\n");
		printf("18. Ultimate로 업그레이드(준비중)\n");
		printf("Q. 나가기\n");
		PRO_BAR
			printf("번호를 입력하세요 : ");
		scanf_s("%hhd", &num);//char형 정수는 %hhd
		...
		case 17:CLS
			if (honor < 1000)
			{
				...
				for (zl = 0; zl < 26; zl++)
				{
					...
					else if (zl != 0 && pl == 3 && (zl + 1) % 24 == 0)		//zl이 0이 아니고 zl에 1을 더한 값이 11로 나누어 떨어지고 zp가 0이라면
					{
						printf("-");																	//(숫자 혹은 영문자 6개 추가 출력후) 두번째 '-' 출력
						actkey[zl] = '-';															//제품키의 zl부분에 '-' 문자를 삽입
						++pl;																		//zp는 2가되어 이 else if 문에 접근이 불가
	...
	nonpro:
		...
		case 13:SUTJA.kum = FALSE; SUTJA.en = FALSE; SUTJA.dong = FALSE;
			...
			while (TRUE)
			{
				sutja_menu(&SUTJA);scanf_s("%hhd", &num);
...
/*Overcalc 시작*/			void start_overcalc(int *plang)
{
	int a;
	printf("\n\n\t\t\tOvercalc Starter\n\n");
	printf("\t\t\tPlease select your language.\n\n");
	printf("1. 한국어\n2.English\n3.한국어-튜토리얼 건너 뛰기\n4.I'm English. Skip tutorial.");
	scanf_s("%d", &a);
	switch (a)
	{
	case 1: *plang = 1;
	case 2: *plang = 2;
	case 3: *plang = 1; return;
	case 4: *plang = 2; return;
	default:;
	}
}
...
/*이스터 에그-일반*/		void Easter_Egg(int *phonor)
{
	char EE[] = "Allen Na";
	char EE_IN[10];
	CLS C_3F
	MENU_BAR		MENU_TITLE		PHONOR		MENU_BAR
	printf("\n공지-일반 버전의 업데이트가 중단되었습니다.\n업데이트를 받으시려면 게임을 해서 얻은 명예점수로\nPRO버전을 구입하시기 바랍니다.\n\n");
	printf("9. 나의 나이는 0000년에 몇 살일까?\n");				//나이 계산기
	printf("2. 라면 끓이는 시간 계산\n");								//라면 시간 계산기
	printf("3. 콩 스탬프 X개는 얼마일까?\n");							//콩스탬프 전환
	printf("4. 미세먼지가 나쁨인가?(간단)\n");						//미세먼지
	printf("5. 미세먼지가 나쁨인가?(일반)\n");
	...
	MENU_BAR
	printf("번호를 입력하세요 : ");
	scanf_s(" %[^\n]%*c", EE_IN, sizeof(EE_IN));
	if (!strcmp(EE_IN, EE)) {
		Easter_EGG(phonor); return;
	}
}

void Easter_EGG(int *phonor)
{
	char EE_IN[50] = { 0 };
	char *pEE = "Overcalc Easter Egg";

	CLS C_3F
	MENU_BAR		MENU_TITLE		PHONOR		MENU_BAR
	printf("\n공지-일반 버전의 업데이트가 중단되었습니다.\n업데이트를 받으시려면 게임을 해서 얻은 명예점수로\nPRO버전을 구입하시기 바랍니다.\n\n");
	printf("5. 나의 나이는 0000년에 몇 살일까?\n");				//나이 계산기
	printf("2. 라면 끓이는 시간 계산\n");								//라면 시간 계산기
	printf("3. 콩 스탬프 X개는 얼마일까?\n");							//콩스탬프 전환
	printf("4. 미세먼지가 나쁨인가?(간단)\n");						//미세먼지
	printf("5. 미세먼지가 나쁨인가?(일반)\n");
...
void EASTER_EGG(int *phonor)
{
	...
	{
		CLS easter_egg++;
		GREEN EASTER	printf("					   	       준비중.\n\n");		END PS CLS
		BLUE EASTER		printf("					   	       준비중..\n\n");	END PS CLS
		WHITE EASTER	printf("					   	       준비중...\n\n");	END PS CLS
		PINK EASTER		printf("					   	       준비중....\n\n");	END PS CLS
		YELLOW EASTER printf("					   	       준비중.....\n\n"); END PS CLS C_0F
		printf("환영합니다. Overcalc의 이스터에그입니다.\n"); PS PS
		printf("그리고..축하드립니다.\n"); PS PS
		printf("이곳까지 오기는 쉽지 않으셨을 텐데 말이죠..\n"); PS PS
		printf("오는 방법을 정확히 설명해 드릴게요. 괜찮으신가요?(Y, N)\n");
		scanf_s("%c", &okay, sizeof(okay));
		switch (okay)
		{
		case 'y':
		case 'Y': help_easter(); break;
		case 'n':
		case'N':break;
		}
		CLS
		printf("그럼 시작합니다.");	PS CLS
		printf("그럼 시작합니다..");	PS CLS
		printf("그럼 시작합니다...");	PS CLS
		while (TRUE)
		{
			...
			if (m[0] > 0 && m[1] > 0 && m[2] > 0 && m[3] > 0 && m[4] > 0 && mk == 0)
			{
				mk++;
				printf("메인으로 돌아가기 메뉴를 제외하고,\n");
				printf("모든 메뉴를 한 번씩 실행하셨군요...\n");
				PT
				printf("자, 이건 선물입니다.");PS PS
				for (int i = 0; i < 25; i++)
				{
					CLS
						switch (i)
						{
						case 20: case 15: case 10: case 5:
						case 0: GREEN break;
						case 21: case 16: case 11: case 6:
						case 1: PINK break;
						case 22: case 17: case 12:	case 7:
						case 2: YELLOW break;
						case 23: case 18: case 13: case 8:
						case 3: WHITE break;
						case 24: case 19: case 14: case 9:
						case 4: BLUE break;
						}		
						if (i < 5) {
							MKMNU printf("					   	     메뉴 준비중\n\n");  END
							...
							MKMNU printf("					   	  기능을 불러오는중\n\n"); END
						}
						else if (i < 25) {
							MKMNU printf("					   	  이스터에그 추가중...\n\n"); END
						}
						if (i<3)
							PT
						else if (i<6)
			...
			if (EE == 1)
			{
				PRO_BAR	EASTER_TIT	PRO_BAR
				printf("1. 이스터에그-1\n");
				printf("2. 이스터에그-2\n");
				printf("3. 이스터에그-3\n");
				printf("4. 이스터에그-4\n");
				printf("5. 이스터에그-5\n");
				printf("E. 이스터에그의 이스터에그\n");
				printf("Q. 메인으로 돌아가기\n");
				scanf_s("%c", &choo, 1);
				switch (choo)
				{
				case '1':
					printf("1번 이스터 에그에 오신것을 환영합니다.\n");
					printf("1번 이스터 에그에 오신것을 환영합니다.\n");
				case '2': printf("2");
				case '3': printf("3");
				case '4': printf("4");
			...
			else
			{
				...
				case 1: ++m[0];
					printf("1번 이스터 에그에 오신것을 환영합니다.\n");
					printf("Premium버전의 명예점수 값이 너무 비싸죠?\n\n"); 
					printf("이 이스터에그는 명예점수를 엄청나게 드릴 겁니다.\n");
					printf("단 1회용이니 참고하세요.\n");
					for (int a = 50; a > 0; a--)
					{
						printf("%d", a);
						_getch();
					...
					*phonor += 1000;
					break;
				case 2: ++m[1]; printf("2"); break;
				case 3: ++m[2]; printf("3"); break;
				case 4: ++m[3]; printf("4"); break;
				case 5: ++m[4]; printf("5"); break;
				case 6: return;
				default: printf("잘못입력");
				}
			}
...
/*Overcalc의 발전*/		void dvlp_overcalc()
{
	...
	printf("Zaicyan(2.5)-Pro버전의 계산기가 무한 반복되는 현상 수정 및 인형뽑기에서 흔들기 추가\n");
	...
	printf("Eastersmalt(3.7)-대규모의 이스터 에그 생성\n");
	printf("Enterare(3.8)-Premium버전 생성\n");
	ll
}
...
void v_overcalc()
{
	CLS
	printf("1. BAISC\n");
	printf("Overcalc를 시작하면 바로 보이는 버전입니다.\n");
	printf("주요 기능으로는 인형뽑기, 가위바위보 게임 등이 있습니다.\n");
	printf("2. PRO\n");
	printf("BASIC버전을 업그레이드 하면 사용할 수 있는 버전입니다.\n");
	printf("주요 기능으로는 로또 번호 출력, 계산기 등이 있습니다.\n");
	printf("3. ENTERPRISE\n");
	printf("4. ULTIMATE\n");
	printf("5. PREMIUM\n");
	printf("6. OVERCALC\n");
...
/*이스터에그 경로 설명*/void help_easter()
{
	CLS
	GREEN printf("우선, 메인화면에서 숫자 999를 입력합니다.\n\n"); PS  PS
	BLUE printf("창이 조금 깜빡이면 프로그램의 제작자 이름을 입력합니다.(Allen Na)\n\n"); PS PS
	WHITE printf("한번 더 깜빡이면 주문을 입력합니다.\n\n"); PS PS
	PINK printf("이스터에그를 부른다는 뜻의 Overcalc Easter Egg.\n\n"); PS PS
	YELLOW printf("단, 대소문자 구별에 주의하시고, 알파벳과 공백만 적습니다.\n\n"); PS PS
	GREEN printf("프로그램 실행 후 최초 1회는 부팅화면이 표시됩니다.\n\n"); PS PS
	BLUE printf("그 뒤로는 이 창을 비롯한 알림 문구가 모두 사라지며,\n\n"); PS PS
	WHITE printf("바로 메뉴화면이 뜨게 됩니다.\n\n"); PS PS
	PINK printf("사소한 오타만 나도 프로그램에 오류가 나니 주의해 주시고,\n\n"); PS PS
	WHITE printf("이제 오는 방법을 잘 아시겠죠?\n\n"); PS PS
	YELLOW printf("Overcalc 보다 재밌는 기능이 더 많은 Overcalc Easteregg로\n\n"); PS PS
	GREEN printf("보내 드리겠습니다!\n\n"); PS PS
	BLUE PUS
}
